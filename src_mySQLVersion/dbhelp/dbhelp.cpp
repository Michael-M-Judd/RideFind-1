...
bool dBHelper::authentication(String^ userId, String^ password) {
	Connect();
	cmd.setCommandText(
		"Select emailId,password from User_Account Where emailId=:1");
...
/*
string getEmail(User u)
{
//set command text as selecting the email
cmd.setCommandText(
"SELECT email_address FROM User_Account WHERE user_id =:1");
cmd.Param(1) = u.getID;
try {
cmd.Execute();
con.Commit();
